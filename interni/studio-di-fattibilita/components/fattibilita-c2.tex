...
\begin{document}
  ...
  Il software da realizzare consiste in un pacchetto di \textit{Node.js}, installabile tramite \glossario{npm} (\textbf{Node Package Manager}), utilizzabile tramite interfaccia a riga di comando.
  Il programma deve offrire le seguenti funzionalità:
  \begin{itemize}
    \item inizializzare \textit{Etherless} sulla propria macchina;
    \item configurare il proprio account \glossario{Ethereum};
    \item caricare le funzioni \glossario{javascript} eseguibili da remoto;
  ...
  \begin{description}
    ...
    \item[\glossario{Typescript} 3.6] per le comunicazioni tra utente e network;
    \item[\glossario{ESlint}] per l'analisi statica del codice;
    \item[\glossario{Serverless Framework}] per l'integrazione Serverless.
  \end{description}
