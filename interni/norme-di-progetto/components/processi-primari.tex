...
Per identificare i requisiti in maniera univoca, Grupp0ne ha deciso di adottare delle norme per la classificazione dei requisiti.
Ogni requisito è caratterizzato da un codice alfanumerico così formato:
\\\\\centerline{\textbf{R[numero][tipo][priorità]}}
in cui ogni elememento ha un diverso significato:
\begin{itemize}
	\item \textbf{Numero}: indica quale numero di caso d'uso si sta esaminando. È un numero progressivo che parte da 1.
	...
	\begin{itemize}
		...
		\item N: legislativo - indica che le regole che il sistema deve rispettare affinchè sia legale.
		...
		\item S: per la sicurezza - indica procedure da adottare affinchè il sistema sia considerabile sicuro.
	\end{itemize}
