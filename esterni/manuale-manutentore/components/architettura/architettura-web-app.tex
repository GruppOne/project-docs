...
\end{figure}
Il token viene creato dal server e poi arriva nell'header della risposta http alla web application, a questo punto la classe LoginService che aveva effettuato la richiesta di login conserva il token jwt in un cookie e il suo payload, ossia l'id dell'utente attuale, viene immagazzinato in \glossario{localstorage}.
...
Abbiamo deciso di sfruttare un meccanismo intrinseco di angular, le \glossarioLocale{Guard}, abbiamo dichiarato infatti una classe \textit{AuthGuard} che ha come scopo il controllo dei permessi dell'utente e, qualora fossero sufficienti, permettere l'accesso alla pagina.
\begin{figure}[H]
  \centering
