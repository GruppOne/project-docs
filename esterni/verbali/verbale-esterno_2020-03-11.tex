...
Lo scopo principale era discutere della possibilità per il gruppo di utilizzare l'infrastruttura server di Imola Informatica.
...
\item Ciao ragazzi, Vi va di spiegarmi un attimo che risorse vi servono e cosa ci andate a fare sopra così cerco di darvi qualche consiglio. Tenete conto che le risorse che vi possiamo dare non hanno garanzie di SLA quindi cerchiamo di pianificare bene le cose
\end{enumerate}
...
\begin{enumerate}
  ...
  Avevamo pensato di utilizzare le vostre risorse come dev environment remoto per il nostro server spring boot che espone un'API RESTful. Quindi ci basterebbe fare ssh sul vostro server e:
  ...
  e poi fare richieste HTTP dai nostri pc usando prima curl e più avanti la web app e app che svilupperemo, in modo da verificare se le cose funzionano come dovrebbero.
...
\begin{enumerate}
  ...
  \item Per il resto vi invito sulla mail che mi avete dato e vi credo gli account per creare delle vm su di un nostro cluster interno
  \item Ma sul principio di creare l intera infrastruttura in locale mi sembra una cosa sulla quale dovreste spingere
  \item Anche in un ottica Dev ops e vi facilità poi a approcciare Kubernetes
\end{enumerate}
...
Per questo e per quanto riferitoci nel primo messaggio il gruppo ha deciso di iniziare a configurare una versione locale dell'applicazione con l'utilizzo di docker prima di accedere al server dell'azienda.
...
\begin{enumerate}
  \item Creare una versione locale del ciclo dell'applicazione con docker, da poter poi utilizzare con \glossario{Kubernetes}
\end{enumerate}
