...
% TODO sposta metriche qui
  \subparagraph{MPSIDG-Indice di Gulpease}%
\label{subp:MPSIDG-indice_di_Gulpease}
...
\end{itemize}
\subparagraph{MPSIGF-Indice di Gunning Fog}%
\label{subp:MPSIGF-indice_di_Gunning_Fog}
...
\subparagraph{MPSCOO-Correttezza ortografica}%
\label{subp:MPSCOO-correttezza_ortografica}
...
Abbiamo deciso di distribuire il prodotto software su più repository corrispondenti alle parti principali di cui è costituito, che verranno creati in corrispondenza dell'approvazione di un'\glossario{API} stabile per il prodotto software che stiamo sviluppando.
...
Ci aspettiamo che questi numeri di versione raggiungano cifre elevate e ci impegniamo a darne una previsione per ogni repository basata sul numero di requisiti concordati con il committente in seguito alla prima revisione di progetto.
...
Valuteremo la convenienza manutentiva di fornire al committente in corrispondenza delle revisioni di progetto un repository impostato per tracciare le parti del sistema attraverso il meccanismo dei \href{https://git-scm.com/book/en/v2/Git-Tools-Submodules}{Submodule} al repository \textit{project-docs} in cui sono mantenuti e versionati i prodotti documentali gestionali e di relazione con il committente.
...
Gli obiettivi e le strategie che il team si impegna ad applicare durante lo svolgimento del progetto sono presentati nel \glossario{piano di qualifica}.
...
% TODO scrivere pr review di github
...
% FIXME livello di indentazione sbagliato
