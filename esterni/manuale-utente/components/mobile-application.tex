...
Nel caso non abbiate un file browser nel vostro dispositivo, vi preghiamo di installarlo dal \textit{Play Store}.
\newpage
...
Se l'utente non possiede delle credenziali per accedere a Stalker, ha la possibilità di registrarsi grazie all'apposito form, cliccando su \textbf{Not yet registered? Sign up} (~\ref{fig:mobile_app_pagina_di_registrazione} ).
\newpage
...
\begin{itemize}
    ...
    \item inserire l'email, che deve essere univoca all'interno del database di Stalker.
    \item inserire la password che deve essere alfanumerica con almeno 8 caratteri e al massimo 32. La password può essere visualizzata normalmente durante la sua digitazione, cliccando sull'occhio barrato alla fine del campo d'inserimento.
...
Se l'utente possiede già delle credenziali e vuole accedere in Stalker, può raggiungere l'apposito form di autenticazione disponibile cliccando \textbf{Already have an account? Login} (~\ref{fig:mobile_app_pagina_di_accesso} ).
\newpage
...
\begin{itemize}
    ...
    \item Cercare tra le applicazioni in lista \textit{Stalker} e selezionarlo.
...
Se il problema è generale, probabilmente avete un problema tecnico e vi preghiamo di contattare il vostro \glossario{\textit{vendor}} al più presto.
