...
\begin{center}
 ...
 \begin{center}
  ...
  \begin{itemize}
    ...
    \item Fork e join.
  \end{itemize}
  I diagrammi di attività si basano sulla produzione e consumo di token. Ogniqualvolta inizia il flusso di esecuzione e quindi ci trova nel nodo iniziale si genera un token. La fork produce un token per ogni processo in parallelo in esecuzione mentre la join li consuma tutti e ne genera uno. Infine il nodo finale consuma tutti i token.
  ...
  \begin{description}
    ...
    \begin{center}
      ...
      \end{minted}
    \end{center}
    \item[Nodi di branching]: i nodi di branching modellano delle decisioni. Non consumano ne generano token in quanto il flusso di esecuzione rimane sempre unico. Si raffigurano mediante rombi vuoti e generalmente distinguono due branch:
    \begin{center}
      \begin{minted}{text}
       @startuml
       (*) --> "Initialization"
    ...
    \begin{center}
     ...
     \end{minted}
   \end{center}
   \item[Segnali]: I segnali servono per gestire eventi. PlantUML non ha un buon supporto per i segnali e ne offre una versione in beta. Ne esistono di due tipi:
   \begin{itemize}
